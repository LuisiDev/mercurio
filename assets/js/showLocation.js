...
actionBtn.addEventListener("click", (e) => {
  ...
  M.toast({
    html: "Obteniendo tu ubicación. Espere por favor...",
    classes: "rounded",
  });
...
getLocation = () => {
  if (navigator.geolocation) {
    ...
    M.toast({
      html: "Lo sentimos, tu navegador web no soporta está función. Por favor, actualiza tu navegador a la versión más reciente.",
      classes: "rounded",
    });
  }
};
...
showError = (error) => {
  ...
  switch (error.code) {
    case error.PERMISSION_DENIED:
      mapArea.innerHTML =
        "<svg class='text-center w-6 h-6 text-red-800' aria-hidden='true' xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24'><path stroke='currentColor' stroke-linecap='round' stroke-linejoin='round' stroke-width='2' d='M12 13V8m0 8h.01M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z'/></svg><br><b class='text-center' style='color:red;'>Denegaste los permisos para obtener tu localización. Para usar esta función, acepte los permisos.</b>";
      break;
    case error.POSITION_UNAVAILABLE:
      mapArea.innerHTML =
        "<br><br><i class='fa-solid fa-circle-exclamation' style='color: red; font-size:50px'></i><br><b style='color:red;'>La información de tu localización no se encuentra disponible en estos momentos, intentelo más tarde.</b>";
      break;
    case error.TIMEOUT:
      mapArea.innerHTML =
        "<br><br><i class='fa-solid fa-circle-exclamation' style='color: red; font-size:50px'></i><br><b style='color:red'>La solicitud de obtener tu ubicación a tardado mucho tiempo. Por favor, intentelo nuevamente.</b>";
      break;
    case error.UNKNOWN_ERROR:
      mapArea.innerHTML =
        "<br><br><i class='fa-solid fa-circle-exclamation' style='color: red; font-size:50px'></i><br><b style='color:red'>A ocurrido un error desconocido. Por favor, intentelo nuevamente.</b>";
      break;
  }
};
...
populateCard = (geoResults) => {
  ...
  // Add text for selecting location
  const selectText = document.createElement("div");
  selectText.innerHTML = "Selecciona tu ubicación";
  selectText.classList.add("text-center", "font-semibold", "text-lg", "mb-2"); // Add class for centering the text
  locationsAvailable.appendChild(selectText);
  ...
  geoResults.map((geoResult) => {
    ...
    input.classList.add("with-gap", "hidden"); // Add the "hidden" class to hide the input
    ...
    // Add Tailwind CSS classes for centering the cards
    // addressCard.classList.add("flex", "justify-center", "items-center");
